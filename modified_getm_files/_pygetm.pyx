...
cdef extern void advection_uv_calculate(int direction, int nk, void* advection, void* tgrid, void* ugrid, double* pu, double* Ah, double timestep, double* pD, double* pDU, double* pvar, double* pchi_num) nogil
cdef extern void advection_w_calculate(void* padvection, void* tgrid, double* pw, double* pw_var, double timestep, double* ph, double* pvar, double* pchi_num) nogil
...
cdef extern void c_update_adaptive(int nx, int ny, int nz, int halox, int haloy, const int* mask, const double* D, const double* NN, const double* SS, double* nu, const double* sdecay, const double* bdecay, int hpow, double chsurf, const double* ihsurf, double chmidd, const double* ihmidd, double chbott, const double* ihbott, double cneigh, double rneigh, double cNN, double drho, double cSS, double dvel, double chmin, double hmin, const double* ga) nogil
...
cdef class Advection:
    ...
    def u_2d(self, Array u not None, double timestep, Array var not None, Array Ah=None, Array chi_num=None):
        ...
        advection_uv_calculate(1, 1, self.p, self.grid.p, self.ugrid.p, <double *>u.p, pAh, timestep, self.ph, self.pDU, <double *>var.p, pchi_num)

    @cython.initializedcheck(False)
    @cython.boundscheck(False) # turn off bounds-checking for entire function
    def v_2d(self, Array v not None, double timestep, Array var not None, Array Ah=None, Array chi_num=None):
        cdef double * pAh = <double *>Ah.p if Ah is not None else NULL
        cdef double * pchi_num = <double *>chi_num.p if chi_num is not None else NULL
        advection_uv_calculate(2, 1, self.p, self.grid.p, self.vgrid.p, <double *>v.p, pAh, timestep, self.ph, self.pDV, <double *>var.p, pchi_num)

    @cython.initializedcheck(False)
    @cython.boundscheck(False) # turn off bounds-checking for entire function
    def u_3d(self, Array u not None, double timestep, Array var not None, Array Ah=None, Array chi_num=None):
        cdef double * pAh = <double *>Ah.p if Ah is not None else NULL
        cdef double * pchi_num = <double *>chi_num.p if chi_num is not None else NULL
        advection_uv_calculate(1, <int>var._array.shape[0], self.p, self.grid.p, self.ugrid.p, <double *>u.p, pAh, timestep, self.ph, self.phu, <double *>var.p, pchi_num)

    @cython.initializedcheck(False)
    @cython.boundscheck(False) # turn off bounds-checking for entire function
    def v_3d(self, Array v not None, double timestep, Array var not None, Array Ah=None, Array chi_num=None):
        cdef double * pAh = <double *>Ah.p if Ah is not None else NULL
        cdef double * pchi_num = <double *>chi_num.p if chi_num is not None else NULL
        advection_uv_calculate(2, <int>var._array.shape[0], self.p, self.grid.p, self.vgrid.p, <double *>v.p, pAh, timestep, self.ph, self.phv, <double *>var.p, pchi_num)

    @cython.initializedcheck(False)
    @cython.boundscheck(False) # turn off bounds-checking for entire function
    def w_3d(self, Array w not None, Array w_var not None, double timestep, Array var not None, Array chi_num=None):
        cdef double * pchi_num = <double *>chi_num.p if chi_num is not None else NULL
        advection_w_calculate(self.p, self.grid.p, <double *>w.p, <double *>w_var.p, timestep, self.ph, <double *>var.p, pchi_num)
...
cdef class VerticalAdvection:
    ...
    def w_3d(self, Array w not None, Array w_var not None, double timestep, Array var not None):
        advection_w_calculate(self.p, self.grid.p, <double *>w.p, <double *>w_var.p, timestep, self.ph, <double *>var.p, NULL)
...
def update_adaptive(Array f not None, Array ga not None, Array NN not None, Array SS not None, const double [::1] sdecay, const double [::1] bdecay, int hpow, double chsurf, const double [:, ::1] ihsurf, double chmidd, const double [:, ::1] ihmidd, double chbott, const double [:, ::1] ihbott, double cneigh, double rneigh, double cNN, double drho, double cSS, double dvel, double chmin, double hmin):
    cdef int halox = f.grid.halox
    cdef int haloy = f.grid.haloy
    cdef int nx = f.grid.nx_
    cdef int ny = f.grid.ny_
    cdef int nz = f.grid.nz_
    cdef Array mask = f.grid.mask
    cdef Array D = f.grid.Dclip
    assert f.z == CENTERS
    assert ga.grid is f.grid and ga.z == INTERFACES
    assert NN.grid is f.grid and NN.z == INTERFACES
    assert SS.grid is f.grid and SS.z == INTERFACES
    assert ihsurf.shape[0] == ny and ihsurf.shape[1] == nx
    assert ihmidd.shape[0] == ny and ihmidd.shape[1] == nx
    assert ihbott.shape[0] == ny and ihbott.shape[1] == nx
    assert sdecay.shape[0] == nz
    assert bdecay.shape[0] == nz
    c_update_adaptive(nx - 2*halox, ny - 2*haloy, nz, halox, haloy, <int*>mask.p, <double*>D.p, <double*>NN.p, <double*>SS.p, <double*>f.p, &sdecay[0], &bdecay[0], hpow, chsurf, &ihsurf[0,0], chmidd, &ihmidd[0,0], chbott, &ihbott[0,0], cneigh, rneigh, cNN, drho, cSS, dvel, chmin, hmin, <double*>ga.p)
...
def tridiagonal(Array nu not None, Array var not None, double dt):
    ...
    assert var.grid is nu.grid and var.z == INTERFACES
    assert nu.z == CENTERS
